"""Administrative helpers and slash commands."""
...
class Admin(commands.Cog):
    ...
    @staticmethod
    def owner_or_admin(inter: discord.Interaction) -> bool:
        """Return ``True`` when the invoker owns the app or has admin rights."""

        application = getattr(inter.client, "application", None)
        owner = getattr(application, "owner", None)
        app_owner_id = getattr(owner, "id", None)

        is_owner = app_owner_id == inter.user.id
        member = inter.user if isinstance(inter.user, discord.Member) else None
        is_admin = any(r.permissions.administrator for r in getattr(member, "roles", [])) if member else False
